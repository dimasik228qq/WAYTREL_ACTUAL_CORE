...
public class Player extends Playable
{
	...
	public boolean doDie(Creature killer)
	{
		...
		{
			PvpbookInfo pkPvpbookInfo = killer.getActingPlayer().getPvpbook().getInfo(getObjectId());
			
			if ((pkPvpbookInfo != null) && !pkPvpbookInfo.isRevenged())
			{
				pkPvpbookInfo.setRevenged(true);
				sendPacket(new SystemMessage(SystemMessageId.YOU_VE_TAKEN_REVENGE_ON_C1).addPcName(this));
				killer.getActingPlayer().sendPacket(new SystemMessage(SystemMessageId.YOU_VE_TAKEN_REVENGE_ON_C1).addPcName(this));
				killer.getActingPlayer().sendPacket(new SystemMessage(SystemMessageId.C1_TOOK_REVENGE_ON_YOU).addPcName(this));
				
				if (pkPvpbookInfo.getKilledObjectId() != getObjectId())
				{
					Player player = pkPvpbookInfo.getKilled();
					if (World.getInstance().getPlayer(player.getObjectId()) != null)
					{
						PvpbookInfo reqPvpbookInfo = player.getPvpbook().getInfo(getObjectId());
						if ((reqPvpbookInfo != null) && !reqPvpbookInfo.isRevenged())
						{
							reqPvpbookInfo.setRevenged(true);
							player.sendPacket(new SystemMessage(SystemMessageId.YOU_VE_TAKEN_REVENGE_ON_C1).addPcName(this));
							player.sendPacket(new SystemMessage(SystemMessageId.C1_TOOK_REVENGE_ON_YOU).addPcName(this));
						}
					}
					else
					{
						getPvpbook().deleteRevengedByHelp(pkPvpbookInfo.getKilledObjectId(), pkPvpbookInfo.getKillerObjectId());
					}
				}
			}
		}
		
		if (!isInsideZone(ZoneId.PVP) && killer.isPlayer())
		{
			PvpbookInfo pvpbookInfo = getPvpbook().addInfo(this, killer.getActingPlayer(), (int) TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis()), 0);
			if (pvpbookInfo != null)
			{
	...
	private static Player restore(int objectId)
	{
			...
			PreparedStatement statement = con.prepareStatement(RESTORE_CHARACTER))
		{
			...
			player.getPvpbook().restore(player);
	...
	public void setCoinTimeout(boolean value)
	{
		this._coinTimeout = value;
	}
	
	public boolean getCoinTimeout()
	{
		return this._coinTimeout;
	}
	...
	{
		if (getAutoPlaySettings().getNextTargetMode() == 4)
		{
